...
class Motor:
    """
    ack probability with a continuous function on nerkh and variance on preservation in minds then ack or nack <<
    continuous means diffrentiation ->
    """
    ...
    def ack_location(self):
        # requests.post(host + '/{}/{}@{},{}'.format(self.hang, self.name, *rnd(tehran)), data={'key': self.key})
        requests.post(host + '/{}/{}@{},{}'.format(self.hang, self.name, *self.location), data={'key': self.key}, timeout=2)
